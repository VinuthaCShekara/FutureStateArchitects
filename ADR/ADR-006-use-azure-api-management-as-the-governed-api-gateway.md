# ADR-006: Use Azure API Management as the Governed API Gateway
...
Visitor channels, staff applications, AI orchestration, and partner integrations require consistent API security, throttling, versioning, routing, and observability.
...
Publish governed external and cross-domain APIs through Azure API Management. Apply OAuth 2.0 and OpenID Connect validation where identity is required, product and subscription controls where appropriate, throttling, quotas, versioning, request validation, correlation propagation, and standardized error responses. Keep business logic out of gateway policies.
...
- Expose application services directly
- Implement gateway concerns separately in each service
- Place business orchestration in APIM policies

## Consequences

- API controls and developer experience are centralized.
- APIM becomes a critical shared dependency and must be sized and deployed according to availability needs.
- Policy sprawl requires governance and automated testing.
...
A gateway outage or policy error can affect many services. Use safe policy deployment, validation, rollback, health probes, capacity monitoring, and direct isolation of internal service traffic where appropriate.

## Implementation and Validation Actions

- [ ] Define API standards and policy fragments.
- [ ] Classify anonymous, customer, workforce, and system APIs.
- [ ] Define versioning and deprecation policy.
- [ ] Validate tier, capacity, networking, and multi-region requirements through NFRs and load tests.
