# ADR-011: Implement the Visitor Assistant Using Grounded RAG Behind an Orchestration Layer
...
Visitors need conversational access to approved estate information, attraction discovery, and itinerary support. Responses must be grounded, safe, observable, and replaceable.
...
Place an AI orchestration service behind API Management. Retrieve approved content through Azure AI Search and generate responses with Azure OpenAI. Enforce source grounding, content safety, prompt-injection defenses, session controls, response policy checks, and human escalation. Keep the model provider behind an abstraction boundary and provide deterministic fallback content.
...
- Use an ungrounded general-purpose chatbot
- Embed model calls directly in every channel
- Use search-only responses without conversational synthesis

## Consequences

- Responses can be tied to approved knowledge.
- Generated output remains non-deterministic and requires continuous evaluation.
- Search indexing, token usage, latency, and model capacity become cost and performance drivers.
...
Hallucination, prompt injection, malicious content, stale knowledge, and cost spikes are mitigated through retrieval scoping, citations, filters, rate limits, token budgets, evaluation, and fallback responses.

## Implementation and Validation Actions

- [ ] Define approved source ownership and publishing workflow.
- [ ] Create offline and online evaluation suites for groundedness, safety, relevance, latency, and fallback rate.
- [ ] Define prohibited topics and escalation journeys.
- [ ] Log safely without retaining unnecessary personal or prompt data.
