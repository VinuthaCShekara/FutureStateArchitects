# ADR-012: Use Separate Recommendation Orchestration with Rules-Based Fallback
...
Personalized suggestions should use visitor preferences, ticket context, live conditions, and feedback without blocking core visitor or ticketing services when ML is unavailable.
...
Implement a Recommendation Service that retrieves authorized context, calls a versioned Azure Machine Learning or approved model endpoint, applies business and safety rules, and returns ranked suggestions. Cache eligible results. If inference or context retrieval fails, return approved rule-based suggestions.
...
- Embed recommendation logic in the visitor channel
- Require live ML inference for every response
- Use generative AI as the sole recommendation mechanism

## Consequences

- The recommendation capability can evolve independently.
- Fallback behavior preserves availability.
- Feature freshness, consent, bias, cold-start behavior, and outcome monitoring require explicit design.
...
Poor or repetitive suggestions can reduce trust. Use guardrails, diversity rules, frequency controls, feedback loops, evaluation, rollback, and transparent preference controls.

## Implementation and Validation Actions

- [ ] Define eligible features and privacy rules.
- [ ] Define model and rules ownership.
- [ ] Measure acceptance, diversity, freshness, fallback, and latency without using sensitive attributes improperly.
- [ ] Version models, features, and decision policies.
