# ADR-003: Separate Enterprise Messaging, Event Distribution, and Telemetry Streaming
...
The platform has different communication semantics: reliable business commands and workflows, lightweight event notification, and high-volume telemetry streams.
...
Use Azure Service Bus for durable business commands, workflow queues, ordered processing where required, and dead-letter handling. Use Azure Event Grid for lightweight domain or platform event distribution. Use IoT Hub and its compatible streaming endpoint, or Event Hubs where independently justified, for telemetry and high-throughput event streams.
...
- Use Service Bus for every event
- Use Event Grid for transactional commands
- Use one shared event stream for all business and IoT traffic

## Consequences

- Each service is used according to its delivery model.
- Operational teams must understand three messaging patterns.
- Schema ownership, routing standards, and end-to-end correlation are required.
...
Duplicate delivery, poison messages, backlog, and accidental data mixing are controlled through idempotency, DLQ operations, quotas, access separation, and monitoring.

## Implementation and Validation Actions

- [ ] Create a message-classification decision tree.
- [ ] Define naming, namespace, partitioning, topic, subscription, and dead-letter conventions.
- [ ] Define event envelope fields including event ID, type, version, source, timestamp, correlation ID, and trace context.
- [ ] Document retry and replay responsibility for each pattern.
