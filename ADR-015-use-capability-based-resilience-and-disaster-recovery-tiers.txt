# ADR-015: Use Capability-Based Resilience and Disaster-Recovery Tiers
...
Ticketing, animal monitoring, visitor insights, recommendations, and the AI assistant have different criticality and recovery needs. Numeric SLOs, RTOs, and RPOs are not yet agreed.
...
Classify capabilities into business-defined resilience tiers before selecting regional topology or service SKUs. Apply bounded retries with exponential backoff and jitter, timeouts, circuit breakers, bulkheads, dead-letter handling, backups, restore testing, and graceful degradation. Keep ticketing and animal-monitoring failure domains isolated from non-critical AI and analytics workloads.
...
- One availability design for all services
- Select active-active for every component before NFR validation
- Rely only on platform default recovery

## Consequences

- Investment aligns with business criticality.
- Final deployment topology remains open until targets and regional constraints are approved.
- Recovery procedures and evidence become part of operational readiness.
...
Premature multi-region design can add cost and consistency complexity. Delayed decisions can create rework. Resolve the NFRs before production topology is finalized.

## Implementation and Validation Actions

- [ ] Run business-impact analysis and agree SLO, RTO, RPO, maximum tolerable data loss, and peak demand per capability.
- [ ] Map each dependency to failure modes and recovery behavior.
- [ ] Select zonal or regional topology through follow-on ADRs.
- [ ] Perform backup restore, failover, dependency-outage, and replay exercises.
