# ADR-001: Adopt a Hybrid Edge and Azure Cloud Architecture
...
Estate operations must continue despite patchy Wi-Fi, while analytics, AI, governance, and long-term storage benefit from Azure-managed services.
...
Use a hybrid architecture. Perform connectivity-sensitive validation, buffering, device-health checks, and limited local decisions on zone-based edge gateways. Use Azure for centralized ingestion, processing, analytics, AI, governance, and durable storage.
...
- Predominantly on-premises architecture
- Direct device-to-cloud connectivity without an edge tier

## Consequences

- Essential telemetry can be retained during connectivity loss.
- Operational complexity increases because edge software and hardware require lifecycle management.
- Cloud dependency is removed from essential local capture but remains for centralized analytics and AI.
...
Patchy connectivity is reduced through store-and-forward. Gateway failure remains a risk and requires health monitoring, local redundancy where justified, and controlled failover.

## Implementation and Validation Actions

- [ ] Define estate zones and gateway sizing.
- [ ] Specify local retention duration and disk-pressure behavior.
- [ ] Define secure replay, deduplication, ordering, and reconciliation rules.
- [ ] Establish remote gateway monitoring, patching, certificate rotation, and replacement procedures.
