# ADR-005: Adopt Modular Business Capabilities with Evolutionary Service Extraction
...
Ticketing, family passes, visitor profiles, recommendations, animal monitoring, notifications, and reporting have distinct responsibilities, but premature microservice fragmentation would add delivery and operational overhead.
...
Begin with clearly separated business modules, explicit APIs/events, independent data ownership boundaries, and deployment seams. Extract a module into an independently deployed service only when scale, ownership, change cadence, resilience, regulatory isolation, or technology needs justify it.
...
- Single tightly coupled monolith
- Microservice per technical function from day one
- Shared database tables across independently deployed services

## Consequences

- The initial platform remains simpler to deploy while preserving evolution paths.
- Architecture tests and governance are needed to prevent boundary erosion.
- Independent scaling is introduced selectively rather than universally.
...
A modular monolith can degrade into a coupled monolith. Enforce dependency rules, contract tests, module-level ownership, and architecture fitness checks.

## Implementation and Validation Actions

- [ ] Define capability ownership and context maps.
- [ ] Prohibit direct cross-boundary table access.
- [ ] Version APIs and events.
- [ ] Record each extraction decision in a follow-on ADR.
